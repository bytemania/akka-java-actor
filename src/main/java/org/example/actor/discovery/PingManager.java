...
class PingManager {
    ...
    private PingManager(ActorContext<Command> context) {
        ...
        this.listingResponseAdapter = context.messageAdapter(Receptionist.Listing.class, ListingResponse::of);
    ...
    private Behavior<Command> behavior() {
        return Behaviors.receive(Command.class)
                ...
                .onMessage(ListingResponse.class, response -> onListing(response.getListing()))
                .build();
    }

    private Behavior<Command> onPingAll() {
        context.getSystem().receptionist().tell(Receptionist.find(PingService.pingServiceKey, listingResponseAdapter));
